 ...
 * modification for rp2040 Aaron Stafford
...
static uint8_t SSD1306_Buffer[SSD1306_WIDTH * 64 / 8];
...
void SSD1306_Index(int counter1) {
	SSD1306_Buffer[counter1/8] |= 1<<(counter1%8);
}
...
void SSD1306_DrawPixel(uint16_t x, uint16_t y, SSD1306_COLOR_t color) {
	...
	/*for 32 pixel display, shift y value to get second half of byte*/
...
void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count)
{
	...
	for(i = 0; i < count; i++)
	dt[i+1] = data[i];
	i2c_write_blocking(i2c_default, SSD1306_I2C_ADDR, dt, count+1, false);
}
...
void ssd1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data)
{
	...
	dt[0] = 0x80;
	dt[1] = data;
	i2c_write_blocking(i2c_default, SSD1306_I2C_ADDR, dt, 2, false);
}
